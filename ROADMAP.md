...
**The conflict:** restoring "anyone, logged in or not, can add/delete bins" exactly as it was would re-open that same Critical vulnerability. That will not be done silently — it's your call. Options:

1. **Keep the sign-in requirement (current state).** Bins become a feature of registered/guest-mode users, consistent with how every other write action in the app already works (mission claims, XP). Smallest gap from "how it was," but doesn't literally restore anonymous write access.
2. **Allow anonymous INSERT only; keep DELETE/UPDATE owner-only.** Restores "anyone can add a bin" without restoring "anyone can delete anyone's bin" (the more damaging half of the original hole). Needs a new migration relaxing the INSERT policy back to `TO public`. Real risk: unlimited anonymous inserts with no per-source throttle (PostgREST doesn't forward caller IP into RLS by default, so a clean rate limit isn't trivial at the DB layer).
3. **Fully revert bins RLS to `USING (TRUE)` for everything.** Exactly restores pre-session behavior. Knowingly reopens a Critical, already-demonstrated vulnerability. Not recommended.

**Priority:** 🔴 Critical (decision-blocking — nothing implemented until you choose)
**Files:** a new `supabase/migrations/000X_bins_policy_decision.sql` once a direction is chosen; `src/native/screens/MapScreen.tsx` only if the UI messaging needs to change for the chosen option.
**Test:** `scripts/verify-rls.mjs` already asserts "anon cannot insert/delete bins" — whichever option is chosen, update that script to match the new intended behavior and re-run live before calling it done.
**Definition of done:** one option chosen and implemented as a migration (not a manual dashboard edit), applied live, `verify-rls.mjs` updated to match and passing.
...
**Verified:** `resetPasswordForEmail` call live-tested against the real project with the PKCE flow and the `bingo://reset-password` redirect — accepted with no error. The deep-link redemption itself can only be confirmed on a real device (can't click an email link from here).
...
**Fix:** configure a custom SMTP provider in **Supabase Dashboard → Project Settings → Authentication → SMTP Settings** (Resend, SendGrid, Postmark, AWS SES, or similar — any transactional email provider). This removes the shared mailer's harsh limit. Requires an account with an email provider, which this repo can't create — same category as the Google Maps keys and classifier hosting: an external account only the project owner can set up.

**Not attempted:** increasing `[auth.rate_limit].email_sent` via `supabase config push` — that command pushes the _entire_ local `config.toml` (including placeholder values like `site_url = "http://127.0.0.1:3000"`) to the live project, risking a silent overwrite of real dashboard settings this repo can't see. Raised with the project owner directly; decision was to wait out the hourly reset for now rather than risk a blind config push.
...
**Test:** after SMTP is configured, re-run the live `resetPasswordForEmail` check (or just use the app's Forgot Password flow) more than twice within an hour and confirm no rate-limit error.
**Definition of done:** custom SMTP configured; `[auth.rate_limit].email_sent` raised to a real value (dashboard, or a carefully-reviewed scoped `config push` once the rest of the live `[auth]` section has been diffed against the dashboard first — not a blind push).
